...
typedef struct MojinStack{
    ...
    long x; 
    long y; 
    short int flag; //Carries the status of the mojin boy
...
/// BEGIN OF BRUTAL LOGIN
    ...
    int exploreSpace(long x, long y,LinkedHash *VMap,MojinStack *mojinsStack,PriorityQueue *VpQ,Game *gameInstance){
        MojinStack *aux = mojinsStack->prox->prox; 
        while(aux != NULL){
            ...
            for(int i = 0; i < 8  ; i++){
                if(aux->areaAvaliable[i] == 0){ //I can only add into map if the area do not exist                    
                    ...
                    fflush(stdout);
                    // scanf(" %*s %*ld %*ld %*ld", &points);                    
                    long xt, yt;
                    // char bundinha[20]; 
                    // int cXe = cx[gameInstance->round%7];  
                    // int cYe = cy[gameInstance->round%7];  
